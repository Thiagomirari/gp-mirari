...
(function () {
  ...
  class CommercialRepository {
    ...
    transition(id, status, actor, note = "") { const proposal = this.proposal(id); const allowed = { draft:["internal_review","cancelled"], negotiation:["internal_review","cancelled"], internal_review:["approved","negotiation"], approved:["sent","negotiation"], sent:["accepted","rejected","negotiation"], accepted:[], rejected:[], cancelled:[] }; if (!proposal || !(allowed[proposal.status] || []).includes(status)) return null; proposal.status = status; proposal.updatedAt = new Date().toISOString(); const summary = note || statusText[status]; proposal.events.unshift({ at: proposal.updatedAt, actor, type: status, summary }); this.recordLeadHistory(proposal.crmOpportunityRef, actor, `Proposta ${proposal.number}: ${summary}.`, proposal.updatedAt); return proposal; }
