...
(function () {
  ...
  function printReport(data, filters) {
    ...
    const funnelTicket = data.scoped.length ? data.scoped.reduce((sum, lead) => sum + leadValue(lead), 0) / data.scoped.length : 0;
    const pdfGrid = popup.document.querySelector(".grid");
    if (pdfGrid) pdfGrid.insertAdjacentHTML("beforeend", `<div class="card"><small>Ticket medio do funil</small><strong>${money(funnelTicket)}</strong></div>`);
  }
  ...
  function render() {
    ...
    const commercialKpis = target.querySelectorAll(".saas-report-grid.five");
    const commercialKpiGrid = commercialKpis.length ? commercialKpis[0] : null;
    if (commercialKpiGrid) {
      commercialKpiGrid.classList.add("six");
      commercialKpiGrid.insertAdjacentHTML("beforeend", card("Ticket medio do funil", money(commercialFunnelTicket), `${commercialLeads.length} oportunidade(s) no funil`));
    }
